...
<form action="{{route('products.store')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="py-4 px-2 w-3/4 mx-auto border-black border-2 rounded-lg">
        ...
        <div class="grid xl:grid-cols-2 xl:gap-6 pt-4">
            {{-- Precio --}}
            <div class="relative z-0 w-full mb-6 group">
                <input type="number" step="any" name="precio" id="precio" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "  />
            ...
            {{-- Cantidad --}}
            <div class="relative z-0 w-full mb-6 group">
                <input type="number" name="cantidad" id="cantidad" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "  />
            ...
            <div class="relative">
                ...
                <input datepicker datepicker-autohide datepicker-format="dd/mm/yyyy" name="fecha_venta" id="fecha_venta" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 " placeholder="Select date">
                @error('fecha_venta')
                    <p class="text-sm text-orange-900 mt-1">*** {{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="relative">
            ...
            @foreach ($marcas as $marca)
                &nbsp;<label class="font-semibold" for="{{ $marca->id }}">
                    <input type="checkbox" id="{{ $marca->id }}" name="marcas[]" value="{{ $marca->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2">
                    {{ $marca->nombre }}
        ...
        <div class="grid xl:grid-cols-3 xl:gap-2 pt-4">
            ...
            <div class="relative z-0 w-full mb-6 group" id="divImg1">
                ...
                <button type="button" id="btnImg1" name="btnImg1" class="bg-blue-500 hover:bg-blue-700 mx-4 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center text-white"><i class="fas fa-plus"></i> Añadir imagen</button>
            </div>
            {{--  --}}
            ...
            <div class="relative z-0 w-full mb-6 group" hidden id="img2" >
                ...
                <input name="imagen2" id="imagen2" accept="image/*" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none" aria-describedby="imagen2" type="file">
                @error('imagen2')
                    <p class="text-sm text-orange-900 mt-1">*** {{ $message }}</p>
                @enderror
            </div>
