...
@section('contenido')
<div class="py-5">
    <form action="{{route('tienda.procesarProducto', $product)}}" method="post">
        @csrf
        ...
        <div class="py-4 px-2 w-3/4 mx-auto border-black border-2 rounded-lg">
            ...
            {{-- Datos del producto --}}
            <label for="" class="mb-4 text-lg">Datos del producto</label>

            <div class="grid grid-cols-4 border-4 rounded p-3 mt-3 bg-white border-gray-200">
                    {{-- Imagen --}}
                    ...
                    <div class="col-span-4 sm:col-span-4 md:col-span-3 md:mx-2">
                        <h1 class="text-md">{{\Str::limit($product->nombre, 102)}}</h1>
                        <h2 class="text-xl font-medium pt-3">{{$product->precio}} €</h2>
                    </div>
                    {{--  --}}
            </div>
            {{--  --}}
            ...
            <div class="mt-10">
                <div>
                    <button type="submit" class="text-white bg-green-500 hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center"><i class="fa-brands fa-paypal"></i> Pasar al pago</button>
                </div>
            </div>
        </div>
    </form>
